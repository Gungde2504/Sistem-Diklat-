<div>
    ...
    <div class="relative overflow-hidden rounded-2xl px-6 py-5 mb-5 text-white
        ...
        <div class="relative">
            ...
            <div class="flex items-start justify-between gap-4 mb-4">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-3 flex-wrap">
                        <span class="inline-flex items-center gap-1.5 text-[10.5px] font-semibold px-2.5 py-1 rounded-full bg-white/20 border border-white/25 backdrop-blur-sm">
            ...
            <div class="grid grid-cols-3 gap-3">
                <div class="bg-white/10 border border-white/15 rounded-xl p-3 text-center backdrop-blur-sm hover:bg-white/15 transition-all duration-200">
                    <div class="flex items-center justify-center gap-1 mb-1">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-white/50" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
    ...
    <div class="flex gap-1.5 bg-stone-100 rounded-2xl p-1.5 mb-5">
        ...
        <button wire:click="setTab('{{ $key }}')"
            class="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-sm font-medium transition-all duration-200
               {{ $tab === $key ? 'text-white font-semibold shadow-[0_2px_8px_-2px_rgba(200,61,0,.4),0_1px_0_rgba(255,255,255,.2)_inset]' : 'text-stone-500 hover:text-stone-700 hover:bg-stone-50' }}"
    ...
    <div class="bg-white rounded-2xl border border-stone-100 overflow-hidden
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                    <tr style="background:linear-gradient(to right,#fafafa,#f5f5f4)" class="border-b border-stone-100">
                ...
                <tbody class="divide-y divide-stone-50">
                    ...
                    <tr class="hover:bg-orange-50/20 transition-colors duration-150">
                        ...
                        <td class="px-5 py-3.5 text-center">
                            @if($a->durasi)
                            <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-orange-50 text-orange-600 border border-orange-200">{{ $a->durasi }} menit</span>
                            @else
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <span class="inline-flex items-center gap-1 text-[10.5px] px-2.5 py-1 rounded-full font-semibold border
                                {{ $a->is_hadir ? 'bg-green-50 text-green-700 border-green-200' : 'bg-stone-100 text-stone-500 border-stone-200' }}">
                                @if($a->is_hadir)
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>
                                Hadir
                                @else
    ...
    <div class="bg-white rounded-2xl border border-stone-100 overflow-hidden
        ...
        <div class="px-5 py-4 border-b border-stone-100">
            ...
            <div class="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
                <div class="h-2 rounded-full transition-all duration-700" style="width:{{ $pct }}%;background:linear-gradient(90deg,#a855f7,#7c3aed)"></div>
            </div>
        </div>
        @endif
            ...
            <div class="px-5 py-4 hover:bg-stone-50/60 transition-colors duration-150">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-stone-400 mt-0.5">{{ $ep->modul?->kategori ?? '-' }}@if($ep->modul?->estimasi_durasi_jam) · {{ $ep->modul->estimasi_durasi_jam }} jam @endif</p>
                        <div class="flex items-center gap-3 mt-2 flex-wrap">
                            @if($ep->started_at)
                            <span class="flex items-center gap-1 text-[10.5px] text-stone-400">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z" /></svg>
                                Mulai: {{ \Carbon\Carbon::parse($ep->started_at)->format('d M Y') }}
                            </span>
                            @endif
                            @if($ep->completed_at)
                            <span class="flex items-center gap-1 text-[10.5px] text-stone-400">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
