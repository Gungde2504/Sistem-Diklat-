            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-stone-50">
                    ...
                    <tr class="hover:bg-stone-50/70 transition-colors duration-150">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-center gap-1.5">
                                @if($p->status === 'pending')
                                @php $namaMandiri = str_replace("'", "\'", $p->nama ?? '-'); @endphp

                                {{-- Setujui --}}
                                <button
                                    @click="$store.deleteModal.show('Setujui Pengajuan', 'Setujui pengajuan diklat mandiri {{ $namaMandiri }}?', () => $wire.approve('{{ $p->id }}'), 'approve')"
                   ...
                   transition-all duration-200">
                                ...
                                {{-- Tolak --}}
                                <button
                                    @click="$store.deleteModal.show('Tolak Pengajuan', 'Tolak pengajuan diklat mandiri {{ $namaMandiri }}?', () => $wire.reject('{{ $p->id }}'), 'reject')"
                                    class="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-semibold
