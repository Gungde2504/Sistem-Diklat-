...
<head>
    ...
    <title>{{ $title ?? 'Portal Pegawai' }} — Sistem Diklat</title>
    ...
    <div class="min-h-screen pb-20 lg:pb-0 lg:flex">
        ...
        <aside class="hidden lg:flex lg:flex-col w-64 flex-shrink-0 bg-white
            ...
            <div class="flex flex-col items-center text-center flex-shrink-0 px-5 py-6"
                style="background:linear-gradient(160deg,#3B9FD1 0%,#1A78B0 50%,#0F5A8C 100%);
             border-radius:28px 28px 0 0">
                ...
                <p class="text-white/60 text-xs mt-0.5">Portal Pegawai</p>
            </div>

            {{-- Nav --}}
            <nav class="flex-1 px-3 py-4 overflow-hidden">
                @include('pegawai.partials.sidebar-nav')
            </nav>
        ...
        <nav class="lg:hidden bg-white border-t border-stone-200 px-2
            ...
            <div class="flex items-center justify-around">
                ...
                <a href="{{ route('pegawai.dashboard') }}"
                    class="group flex flex-col items-center gap-0.5 px-3 py-2.5 rounded-2xl transition-all duration-200
                      {{ request()->routeIs('pegawai.dashboard') ? 'text-blue-600' : 'text-stone-400 hover:text-blue-500 active:scale-95' }}">
                    <span class="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-4.5 h-4.5 {{ request()->routeIs('pegawai.dashboard') ? 'text-white' : 'text-stone-500 group-hover:text-blue-500' }}"
                            fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                        </svg>
                    </span>
                    <span class="text-[10px] font-semibold {{ request()->routeIs('pegawai.dashboard') ? 'text-blue-600' : 'text-stone-400 group-hover:text-blue-500' }}">Home</span>
                </a>

                {{-- Acara --}}
                <a href="{{ route('pegawai.acara') }}"
                    class="group flex flex-col items-center gap-0.5 px-3 py-2.5 rounded-2xl transition-all duration-200
                      {{ request()->routeIs('pegawai.acara*') ? 'text-blue-600' : 'text-stone-400 hover:text-blue-500 active:scale-95' }}">
                    <span class="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200
                             {{ request()->routeIs('pegawai.acara*')
                                 ? 'shadow-[0_3px_10px_-2px_rgba(15,79,122,.35),0_1px_0_rgba(255,255,255,.2)_inset]'
                                 : 'bg-stone-100 group-hover:bg-blue-50 group-hover:shadow-[0_2px_8px_-2px_rgba(15,79,122,.2)]' }}"
                        @if(request()->routeIs('pegawai.acara*'))
                        style="background:linear-gradient(135deg,#3B9FD1 0%,#1A78B0 50%,#0F5A8C 100%)"
                        @endif>
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-4.5 h-4.5 {{ request()->routeIs('pegawai.acara*') ? 'text-white' : 'text-stone-500 group-hover:text-blue-500' }}"
                            fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                        </svg>
                    </span>
                    <span class="text-[10px] font-semibold {{ request()->routeIs('pegawai.acara*') ? 'text-blue-600' : 'text-stone-400 group-hover:text-blue-500' }}">Acara</span>
                </a>

                {{-- Jam Saya — center button --}}
                <a href="{{ route('pegawai.rekap-jam') }}"
                ...
                transition-all duration-200"
                    ...
                    <span class="text-[10px] font-bold text-blue-600">Jam Saya</span>
                </a>

                {{-- Sertifikat --}}
                <a href="{{ route('pegawai.sertifikat') }}"
                    class="group flex flex-col items-center gap-0.5 px-3 py-2.5 rounded-2xl transition-all duration-200
          {{ request()->routeIs('pegawai.sertifikat') ? 'text-blue-600' : 'text-stone-400 hover:text-blue-500 active:scale-95' }}">
                    <span class="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200
                 {{ request()->routeIs('pegawai.sertifikat')
                     ? 'shadow-[0_3px_10px_-2px_rgba(15,79,122,.35),0_1px_0_rgba(255,255,255,.2)_inset]'
                     : 'bg-stone-100 group-hover:bg-blue-50 group-hover:shadow-[0_2px_8px_-2px_rgba(15,79,122,.2)]' }}"
                        @if(request()->routeIs('pegawai.sertifikat'))
                        style="background:linear-gradient(135deg,#3B9FD1 0%,#1A78B0 50%,#0F5A8C 100%)"
                        @endif>
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-4.5 h-4.5 {{ request()->routeIs('pegawai.sertifikat') ? 'text-white' : 'text-stone-500 group-hover:text-blue-500' }}"
                            fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 0 1-1.043 3.296 3.745 3.745 0 0 1-3.296 1.043A3.745 3.745 0 0 1 12 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 0 1-3.296-1.043 3.745 3.745 0 0 1-1.043-3.296A3.745 3.745 0 0 1 3 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 0 1 1.043-3.296 3.746 3.746 0 0 1 3.296-1.043A3.746 3.746 0 0 1 12 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 0 1 3.296 1.043 3.746 3.746 0 0 1 1.043 3.296A3.745 3.745 0 0 1 21 12Z" />
                        </svg>
                    </span>
                    <span class="text-[10px] font-semibold {{ request()->routeIs('pegawai.sertifikat') ? 'text-blue-600' : 'text-stone-400 group-hover:text-blue-500' }}">Sertifikat</span>
                </a>

                {{-- Profil --}}
                <a href="{{ route('pegawai.profil') }}"
                    class="group flex flex-col items-center gap-0.5 px-3 py-2.5 rounded-2xl transition-all duration-200
                      {{ request()->routeIs('pegawai.profil') ? 'text-blue-600' : 'text-stone-400 hover:text-blue-500 active:scale-95' }}">
                    <span class="w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200
                             {{ request()->routeIs('pegawai.profil')
                                 ? 'shadow-[0_3px_10px_-2px_rgba(15,79,122,.35),0_1px_0_rgba(255,255,255,.2)_inset]'
                                 : 'bg-stone-100 group-hover:bg-blue-50 group-hover:shadow-[0_2px_8px_-2px_rgba(15,79,122,.2)]' }}"
                        ...
                        @endif>
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-4.5 h-4.5 {{ request()->routeIs('pegawai.profil') ? 'text-white' : 'text-stone-500 group-hover:text-blue-500' }}"
                            fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                        </svg>
                    </span>
                    <span class="text-[10px] font-semibold {{ request()->routeIs('pegawai.profil') ? 'text-blue-600' : 'text-stone-400 group-hover:text-blue-500' }}">Profil</span>
                </a>
